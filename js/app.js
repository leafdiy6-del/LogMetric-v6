...
window.onload = () => {
    ...
    function createHistoryViewerRow(log, idx) {
        ...
        const vol = (log.volume != null && !isNaN(log.volume)) ? Number(log.volume).toFixed(3) : '0.000';
    ...
    function updateHistoryViewerItem(id, field, value) {
        ...
        if(field === 'length' || field === 'diameter') {
            ...
            if(vEl) vEl.innerText = (item.volume != null && !isNaN(item.volume)) ? Number(item.volume).toFixed(3) : '0.000';
    ...
    function updateHistoryViewerStats() {
        ...
        if(statsEl) {
            const t = I18N[currentLang];
            statsEl.innerHTML = `<span>${t.total_count}: <strong id="hvTotalCount">${validLogs.length}</strong></span><span>${t.total_vol}: <strong id="hvTotalVol">${totalV.toFixed(3)}</strong> m³</span>`;
        }
    }
    ...
    function updateProDualVolume() {
        ...
        if(vDisplay) vDisplay.innerText = log.volume.toFixed(3);
    ...
    function updateProVolumeDisplay() {
        ...
        el.textContent = totalV.toFixed(3);
        el.title = (I18N[currentLang].total_vol || '材积') + ': ' + totalV.toFixed(3) + ' m³';
    }
    ...
    function calculateVolume(length, diameter) {
        ...
        if (!isNaN(l) && !isNaN(d) && l > 0 && d > 0) {
            ...
            return parseFloat((Math.PI * Math.pow(radius_m, 2) * l).toFixed(3));
        }
        return 0;
    }
    ...
    function updateVolumeDisplay(id, volume) {
        const volumeStr = volume.toFixed(3);
    ...
    function handleDualDiameterVolume(item, id) {
        ...
        if(d1 > 0 && d2 > 0) {
            ...
            if(length > 0) {
                ...
                const vDisplay = document.getElementById('v-' + id);
                if(vDisplay) vDisplay.innerText = item.volume.toFixed(3);
            }
        }
    }
        ...
        let summaryHtml = `
            ...
            <div style="background:#f0f0f0; padding:15px; border:2px solid #000; margin-top:8px; width:100%; box-sizing:border-box; overflow-wrap:break-word; word-wrap:break-word;">
                ...
                <p style="margin:5px 0; font-size:12px;">
                    ...
                    <strong>${t.total_vol}:</strong> <span style="font-weight:bold;">${totalV.toFixed(3)} m³</span>
                </p>
        `;
    ...
    async function exportData(options = {}) {
        ...
        if(showPriceCsv && totalBeforeTax > 0) {
            ...
            // 不显示价格时的简单汇总
            wsData.push([t.total_count, excelRootCount, '', t.total_vol, totalV.toFixed(3) + ' m³']);
            if (excelRowCount !== excelRootCount) {
                wsData.push([t.total_rows, excelRowCount, '', '', '']);
            }
        }
